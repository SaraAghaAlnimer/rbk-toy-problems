...
function tallEnoughToRide (arr){
  return reduce(arr, function(acc, obj){
            if(obj.heightInInches>=48){
             acc.push(obj.name)}
            return acc
  }, [])
}
...
function tallestPerson (arr){
  var tallest =arr.filter(obj=> Math.max(obj.heightInInches))
  return tallest[0].name +" at "+tallest[0].heightInInches+" inches"
}